...
- =set_session_status= - Report a session's run status (idle/working/blocked) to Emacs (driven by Claude Code hooks; see [[#session-status][Session Status]])
...
This works automatically once the built-in tools are enabled with
=claude-code-ide-emacs-tools-setup=: the package registers a =set_session_status=
MCP tool and hands the CLI a small hooks file (shipped with the package) via
=--settings=, which calls that tool on the relevant lifecycle events. No manual
hook configuration is required. Status is cleared automatically when a session
...
To opt out, set =claude-code-ide-report-status= to =nil=; sessions then start
without the status hooks.

#+begin_src emacs-lisp
;; Disable status reporting (enabled by default)
(setq claude-code-ide-report-status nil)
#+end_src
