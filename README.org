...
(use-package claude-code-ide
  ...
  :config
  (setq claude-code-ide-enable-mcp-server t   ; Optionally expose Emacs MCP tools
...
| ~claude-code-ide-enable-emacs-tools~            | Expose built-in Emacs nav tools (xref, etc.) | ~nil~                                  |
| ~claude-code-ide-report-status~                 | Report session run status to Emacs           | ~nil~                                  |
...
The tools are always registered, but only exposed to Claude when the MCP
tools server and the tools themselves are enabled. Add to your configuration:

#+begin_src emacs-lisp
;; Start the MCP tools server and expose the built-in navigation tools
...
=claude-code-ide-report-status= to =t= (the MCP tools server must also be
enabled):

#+begin_src emacs-lisp
;; Enable status reporting (off by default)
(setq claude-code-ide-enable-mcp-server t
      claude-code-ide-report-status t)
#+end_src
...
;; Enable the MCP tools server so custom tools are exposed to Claude
(setq claude-code-ide-enable-mcp-server t)
#+end_src
