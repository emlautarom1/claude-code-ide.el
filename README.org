...
Each session's current *run status* is shown in Emacs, so you can tell at a
glance which sessions need you and which are busy. There are three statuses,
mirroring those the Claude Code CLI reports:

- =waiting= - waiting on you (e.g. a permission prompt; the reason is shown)
- =busy= - actively processing a prompt or running tools
- =idle= - finished and waiting for input

The status is read directly from the per-session files the CLI maintains under
=~/.claude/sessions/= (or =$CLAUDE_CONFIG_DIR/sessions/=): the package watches
that directory and mirrors each running session's status onto its project. No
configuration, hooks, or MCP tools server is required, and the status is
cleared automatically when a session exits, even on a crash.

=M-x claude-code-ide-list-sessions= lists sessions ordered by status (waiting
first, then idle, then busy); each candidate is the project directory. When
[[https://github.com/minad/marginalia][=marginalia=]] is loaded, every row is
annotated -- in aligned columns -- with the session's name, how long it has held
its status, its run status and, when waiting, the reason (e.g. =permission
prompt=).  =marginalia= is the only source of these annotations, so without it
the picker shows just the directory. The status accessors =claude-code-ide-session-run-status= and
...
If you use [[https://github.com/minad/consult][=consult=]], the integration
activates automatically whenever =consult= is loaded -- no extra setup required.
A session source is added to =consult-buffer= (narrow with =c=) that previews
sessions in the side window, and =claude-code-ide-list-sessions= is upgraded to a
=consult=-based picker with that same live preview. It adds no hard dependency
and falls back to plain =completing-read= when =consult= is not installed. (Row
annotations come from =marginalia=, independently of =consult= -- see above.)
