...
- *Any Emacs command or function* can be advertised as an MCP tool, allowing Claude to:
...
(use-package claude-code-ide
  ...
  :custom
  (claude-code-ide-enable-mcp-server t) ; Enable the MCP server
  (claude-code-ide-enable-emacs-tools t)) ; Enable the Emacs tools - requires MCP
#+end_src
...
| =M-x claude-code-ide-send-prompt=             | Send a prompt with the region/buffer as context, then submit |
...
| =M-x claude-code-ide-list-sessions=           | List sessions (ordered by status) and switch                 |
| =M-x claude-code-ide-check-status=            | Check if Claude Code CLI is installed and working            |
| =M-x claude-code-ide-insert-region-or-buffer= | Insert the region or buffer as an @-reference (no submit)    |
| =M-x claude-code-ide-yank=                    | Paste the kill-ring/clipboard into the session (no submit)   |
...
| ~claude-code-ide-enable-emacs-tools~            | Advertise built-in Emacs nav tools (xref…)   | ~nil~                                  |
| ~claude-code-ide-report-status~                 | Report session run status through Claude hooks | ~nil~                                |
| ~claude-code-ide-mcp-server-port~               | Port for MCP tools server                    | ~nil~ (auto-select)                    |
| ~claude-code-ide-mcp-server-tools~              | List of advertised Emacs functions           | ~nil~                                  |
...
| ~claude-code-ide-enable-execute-code~           | Allow model to evaluate Elisp in Emacs       | ~nil~                                  |
...
|--------------+-----------------------------------------------------|
| =S-<return>= | Insert a newline in the prompt (without submitting) |
| =C-<escape>= | Send an escape key to cancel the current operation  |
...
Claude Code can evaluate Elisp expressions directly in your running Emacs session via the =executeCode= MCP tool. This is disabled by default.
...
Claude Code IDE includes built-in MCP tools that advertise Emacs functionality to Claude, enabling powerful code navigation and analysis capabilities:
...
- =set-session-status= - Report a session's run status (idle/working/blocked) to Emacs (driven by Claude Code hooks; see [[#session-status][Session Status]])
...
(setq claude-code-ide-enable-emacs-tools t) ; Enable the Emacs tools - requires MCP
#+end_src
...
*** Session Status
:PROPERTIES:
:CUSTOM_ID: session-status
:END:

Each session reports its current *run status* to Emacs, so you can tell at a
glance which sessions need you and which are busy. There are three statuses:

- =blocked= - waiting on you (a permission prompt or elicitation dialog)
- =working= - actively processing a prompt or running tools
- =idle= - finished and waiting for input

This works once the MCP tools server is enabled (=claude-code-ide-enable-mcp-server=)
and status reporting is turned on (=claude-code-ide-report-status=): the package
registers a =set-session-status= MCP tool and hands the CLI a small hooks file
(shipped with the package) via =--settings=, which calls that tool on the
relevant lifecycle events. No manual hook configuration is required. Status is
cleared automatically when a session exits, even on a crash.

=M-x claude-code-ide-list-sessions= orders sessions by status (blocked first,
then idle, then working) and annotates each with its status and how long it has
held it. The status accessors =claude-code-ide-session-run-status= and
=claude-code-ide-session-run-status-since= are public, so you can surface the
information elsewhere (mode line, tab bar, etc.).

Status reporting is off by default. To enable it, set
=claude-code-ide-report-status= to =t= (the MCP server must also be enabled):

#+begin_src emacs-lisp
(setq claude-code-ide-enable-mcp-server t) ; Enable the MCP server
(setq claude-code-ide-report-status t)     ; Report status through Claude hooks
#+end_src

**** Consult integration (optional)

If you use [[https://github.com/minad/consult][=consult=]] (and optionally
[[https://github.com/minad/marginalia][=marginalia=]]), the integration activates
automatically -- no extra setup required. When =consult= is loaded, a session
source is added to =consult-buffer= (narrow with =c=) that previews sessions in
the side window and shows status/elapsed-time annotations, and
=claude-code-ide-list-sessions= is upgraded to a =consult=-based picker with that
same live preview. It adds no hard dependency and falls back to plain
=completing-read= when =consult= is not installed.

To opt out and keep the plain =completing-read= picker, set
=claude-code-ide-consult-integration= to =nil= before =consult= is loaded (for
example via =use-package='s =:custom=):

#+begin_src emacs-lisp
(setq claude-code-ide-consult-integration nil)
#+end_src

** Creating Custom MCP Tools

You can advertise your own Emacs functions to Claude through the MCP tools system. This allows Claude to interact with specialized Emacs features, custom commands, or domain-specific functionality.
...
(claude-code-ide-make-tool
 ...
 :name "tool-name"             ; Name for Claude to use (kebab-case recommended)
...
(claude-code-ide-make-tool
 ...
 :name "my-project-grep"
 :description "Search for pattern in project files"
 :args '((:name "pattern"
          :type string
          :description "Pattern to search for")))

;; Enable the MCP server so custom tools are advertised to Claude
