...
| =M-x claude-code-ide-insert-region-or-buffer= | Insert region/buffer @-reference with context (no submit)    |
| =M-x claude-code-ide-yank=                    | Paste kill-ring/clipboard with context (no submit)           |
