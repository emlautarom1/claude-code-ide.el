...
annotated with the session's name, its status and any reason (e.g. =waiting ·
permission prompt=), and how long it has held that status -- =marginalia= is the
only source of these annotations, so without it the picker shows just the
directory. The status accessors =claude-code-ide-session-run-status= and
