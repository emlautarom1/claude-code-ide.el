...
=M-x claude-code-ide-list-sessions= orders sessions by status (waiting first,
then idle, then busy) and annotates each with its name, its status and any
reason, and how long it has held that status. The status accessors
=claude-code-ide-session-run-status= and =claude-code-ide-session-run-status-since=
are public, so you can surface the information elsewhere (mode line, tab bar,
etc.).
...
the side window and shows name, status and elapsed-time annotations, and
